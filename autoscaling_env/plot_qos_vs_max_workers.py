...
def main():
    ...
    parser.add_argument("--save", type=str, default="qos_vs_max_workers_scatter.png")
    ...
    for agent in selected:
        ...
        for r in records:
            s = r.get("summary", {})
            if "max_workers" in s and "final_qos_total" in s:
                xs.append(s["max_workers"])
    ...
    plt.xlabel("Max Workers per Episode")
    plt.ylabel("Final QoS")
    plt.ylim(0.0, 1.1)
    plt.title(f"Final QoS vs Max Workers ({args.max_runs} runs)")
